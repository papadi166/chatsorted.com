...
div#tags
    ...
    #add-tags(v-if="add_tags_clicked || store.tags[0] " class="flex-column px-2 ")
        #add-tags-options(class="flex justify-around mt-6 h-7")
            div(class="flex flex-row w-5/6 rounded-2xl bg-gray-100 border-opacity-50 border-solid outline outline-gray-400  outline-1 rounded-lg border-2")
            ...
            div(class="flex flex-row w-3/6 rounded-2xl ml-3 bg-gray-100 border-opacity-50 border-solid outline outline-gray-400  outline-1 rounded-lg border-2")
                div.placeholder(class="w-1/4 flex justify-center items-center")
                    SelectArrowsIcon
        ...
        div.tag-cards(class="h-96 mt-5 w-full rounded-2xl"  ) 
            dig.tag-card(v-for="card in store.tags" :key="card.id" @click="focused = card.id" class="flex-column  min-h-min mt-5  w-full rounded-2xl" :style="`background-color: ${card.color}`"  )
                div.tag-card-header(class="flex w-full h-12 justify-around px-4 py-2 items-center" )
                    
                    input.tag-card-input(class="w-full h-2/5 focus:outline-0  indent-1 mb-2 text-white" :value="card.tag_name" :style="`background-color: ${card.color}`" v-show="card.editing")
                ...
                .tag-card-footer(class="flex h-10 px-2 " v-show="!card.folded")
                    ...
                    button(@click="store.tags.splice(card.id, 1)")
